...
function ObtainDimSize(wv, zNum)
	wave wv
	variable zNum
	variable xNum, yNum, zNumLimit, pts
	xNum=dimsize(wv,1);
	yNum=dimsize(wv,2);
	zNumLimit=dimsize(wv,3)
	pts=dimsize(wv,0);
	if (zNum>zNumLimit)
		print "Inadequate Z Number"
	endif
	return xNum, yNum, pts
end


function MakeFitImages(wv,wcoef, axis, zNum)
	// 現状, 時間がかかりすぎるので, 以下の手順で処理
	...
	xNum, yNum, Pts = ObtainDimSize(wv, zNum)
	...
	// wave4dto2d
	string NameOfWave2D = nameOfWave(wv) + "_2D"
	Wave2Dto4DForFit(wv,xNum,yNum,zNum)
	NumberOfSpatialPts = dimsize($NameOfWave2D)
	...
	i=0
	do
		//Loop for spatial position
		j=0
		do	
			// define image name 
			//こんな感じで, 先にガウスの数だけ1次元wave 作っておく
			FitImage1Dname="FitImage1DGauss"+num2str(i)
			make/O/N=(NumberOfSpatialPts)/D $FitImage1Dname
			wave FitImage1Dname = $FitImage1Dname
			
			temp = $NameOfWave2D[p][j]
			make/t/o/n = (NumofGauss) tempConstraints = Constraints
			wave ProcessedWCoef = CoefProcess(WCoef)
			wave processedWcoef = LinearBaseline(frompix, endpix, temp, axis)
			Funcfit/Q/H=FittingParameters[NumOfGauss] gaussfunc ProcessedWCoef temp[frompix,endpix] /X=axis/D /C=tempConstraints;
			// ガウスの数だけイメージチャンクにしたい
			// ガウスの数だけ, イメージチャンクを先に作成し, 
			FitImage1Dname[j] = processedWcoef[i*3+2]
			j+=1
		while(j<NumberOfSpatialPts)
		i+=1
	while(i<NumOfGauss)
end
		
Function Wave2Dto4DForFit(wv,Numx,Numy,Numz)
// to do delete datatype switch	
	wave	wv;
	variable	Numx,Numy,Numz,DataType;
	variable	i,j,k,wvNum;
	variable start,startnum,endnum, ZCenter, nextstartnum,nextendnum
	
	// make destination wave. the name is wavename + _2D
	wvNum = dimsize(wv, 0)
	string DestWvName = nameOfWave(wv) + "_2D"
	make/O/N=(wvNum,Numx,Numy,Numz)/D DestWvName;
