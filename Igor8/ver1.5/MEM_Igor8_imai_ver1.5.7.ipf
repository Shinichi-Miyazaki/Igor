...
Function/wave MEM_prepMS(rawwv, axiswv, axmin, axmax, out_raw_name, out_axis_name)
	...
	//axiswv
	matrixop/free/o axiswv = abs(axiswv)
	//for the axis which possess negative values
	matrixOP/free/o filterwv = within(axiswv, axmin, axmax)
	matrixop/free/o  even_or_odd = sum(filterwv)
	if(mod(even_or_odd[0], 2)==1)
		k=0
		do
			 tempvar = filterwv[k]
			 k = k+1
	...
	matrixop/free/o newaxis = axiswv * filterwv
	//filtering
	...
	//rawwv
	matrixop/free/o filterwv_raw = colrepeat(filterwv, ncolumn)
	//make 2D filter
	matrixop/free/o rawwv = rawwv + 0.1
	matrixop/free/o temprawwv = rawwv * filterwv_raw
	//filtering
	make/free/O/N=(numaxis, ncolumn) newraw
	variable j
	j=0
	do
		duplicate/free/o/r = [0,*][j] temprawwv, tempx
		...
		j = j +1
	while(j<ncolumn)
	matrixop/free/O newraw = newraw - 0.1
