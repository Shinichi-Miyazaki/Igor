...
Function/wave MEM_prepMS(rawwv, axiswv, axmin, axmax)
	wave rawwv, axiswv
	...
	matrixop/o tempwv = axiswv * filterwv
	tempwv = tempwv[p] == 0 ? NaN : tempwv[p]
	WaveTransform zapNaNs tempwv
	variable numaxis = numpnts(tempwv)
	
	//rawwv
	matrixop/o filterwv_raw = colrepeat(filterwv, ncolumn)
	matrixop/o temprawwv = rawwv * filterwv_raw
	//Extract temprawwv, tempraw_wo_nan, numtype(temprawwv)==0
	make/O/N=(numaxis, ncolumn) raw
	variable startnum, endnum, j
	j=0
	do 
		duplicate/o/r = [0,*][j] temprawwv, tempx
		redimension/N = (numaxis) tempx
